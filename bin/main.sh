# USER OPTIONS
ANALYSE_PATH="path/to/workingdir"
CONFIG_FILE="path/to/configfile.ini"
SOFTWARE_PATH="path/to/software/new_HiFiBR"
...
#			Align first time - mismatch 1% (remain parameters default)
		...
		geneious -i ${sample}_all.fq $ref_name -x $SOFTWARE_PATH/script/DSB_Junc_Align1.optionprofile \
...
#			Align second time max gap - gap 1000 (remain parameters default)	
		...
		geneious -i ${sample}_Unaligned1.sam $ref_name -x $SOFTWARE_PATH/script/DSB_Junc_Align2.optionprofile \
		...
		if [ $keepUnmapped == "True" ]; then
			rm ${sample}_1st.sam ${sample}_2nd.sam
			samtools view -H ${sample}_2nd.sam > ${sample}_Unaligned2.sam
			samtools view -f 4 ${sample}_2nd.sam >> ${sample}_Unaligned2.sam
		elif [ $keepUnmapped == "False" ]; then
			rm ${sample}_Unaligned1.sam ${sample}_1st.sam ${sample}_2nd.sam
		fi
...
#-------------------------------------------------------------------------	
# 4. 3_HiFiBR
	echo "STEP 3: ANALYZING DSB Junctions"
#	Prepare
	cp $SOFTWARE_PATH/script/alt_Hi4_3.py $ANALYSE_PATH/$sample/3_HiFiBR
	cp $ANALYSE_PATH/$sample/2_mapping/${sample}_Padded.sam $ANALYSE_PATH/$sample/3_HiFiBR
	
#	Processing
	cd $ANALYSE_PATH/$sample/3_HiFiBR
	python $SOFTWARE_PATH/script/alt_Hi4_3.py ${sample} ${ANALYSE_PATH} $ref_path $SOFTWARE_PATH $cutPos1 $cutPos2
	cd ../
done
