...
def check_non_temp_ins(subcigar_L, read_seq):
...
def execute_each_line(line, recon_L, no_dup_L, read_err_D, read_err_distri_D):
    cigar = line[5]              
    ...
    if char_L == ["M","I","M"]:
        ...
        M_l, M_r, dist2brk_l, dist2brk_r, del_l, del_r, del_total, ins_start, ins_end, ins_len, ins_seq, uncut_seq_l, uncut_seq_r, recon_cigar = read_len, read_len, 0, 0, 0, 0, 0, 0, 0, 0, "", "", "", ""             
        seq_class = report_class(del_total, ins_len, read_seq, ref_seq, 1)
        check_uncut = "Maybe" 
    else:               
        subcigar_L = check_non_temp_ins(subcigar_L, read_seq)     # diff: non_temp 1st, fake_ins 2nd                       
    ...
    if seq_class == "insertion":
        if M_l + M_r != ref_len:
            ...
            seq_class = "insertion uncut 2 sides"
        ...
        if (seq_class == "exact") and (check_uncut == "Maybe"):
            ...
            if ins_len >0:
                seq_class = "complex uncut 2 side"
            else:
...
for f in input_files:   
    ...
    for i in tqdm(range(0,len(line_L), 10)):
        sub_lines = line_L[i:i+10]
        with ccf.ProcessPoolExecutor() as executor:
            res = [executor.submit(execute_each_line, line, recon_L, no_dup_L, 
                            read_err_D, read_err_distri_D) for line in sub_lines]        
    ...
    tsv_reader = csv.reader(open(f[:-4] + "_Final.tsv", "rt", encoding= "utf-8"), delimiter='\t')  
    workbook = Workbook(f[:-4] + "_Final.xlsx")
    ...
    # Close the XLSX file.
    workbook.close()
    os.remove(f[:-4] + "_Final.tsv")

# D. ADD INFO TO FINAL.XLSX
    # 1. Add "represented read"
os.chdir(path + "/" + directory)
df_final = pd.read_excel(sample + "_Padded_Final.xlsx", engine='openpyxl')
...
df_full = pd.read_csv(sample + "_Padded_Extended.tsv",sep = "\t",header = None)
df_full.columns = col
...
df_info.rename(columns={"Full Name": "Represented read"},inplace = True)
    ...
    # 3. Add "Time of events (file) id" & Get "Time of events (file)" sam file
        # Add id
...
# os.remove(sample + "_padded_vs_altref_Final.xlsx")
df_info.to_excel(sample + "_padded_vs_altref_Final2.xlsx",index=False)
